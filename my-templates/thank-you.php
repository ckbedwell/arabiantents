...
// Debug: Check the raw JSON string
if (!empty($_POST["furniture_items"])) {
  // Remove escape characters
  $json_string = stripslashes($_POST["furniture_items"]);
  $furniture_data = json_decode($json_string, true);


  foreach ($furniture_data as $furniture_item) {
    $unit_price = $furniture_item['price'];
    if ($unit_price) {
      $price = '£' . ($unit_price * $furniture_item['quantity']);
    } else {
      $price = "POA";
    }
    ...
    $furniture_items .= '
    <tr>
      <td><img src="' . htmlspecialchars($furniture_item['featured_image']) . '" alt="' . htmlspecialchars($furniture_item['title']) . '" style="width: 100px; height: auto;"></td>
      <td>' . htmlspecialchars($furniture_item['title']) . '</td>
      <td>' . htmlspecialchars($furniture_item['quantity']) . '</td>
